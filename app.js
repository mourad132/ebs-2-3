...
mongoose.connect('mongodb+srv://mourad132:Momo2005@ebs-2-3.gmuz42k.mongodb.net/?retryWrites=true&w=majority', { useNewUrlParser: true, useUnifiedTopology: true})
var conn = mongoose.createConnection('mongodb+srv://mourad132:Momo2005@ebs-2-3.gmuz42k.mongodb.net/?retryWrites=true&w=majority', { useNewUrlParser: true, useUnifiedTopology: true});
...
app.get('/profiles', ensureAuthenticated, function(req, res){
	//Regex Search
        ...
        User.find({name: regex, type: 'student'}, function(err, users){
           if(err){
               console.log(err);
           } else {
              if(users.length < 1) {
                  noMatch = "No Students match that search, please try again.";
              }
              res.render("profiles", {profiles: users, noMatch: noMatch, user: req.user, page: "Students"});
           }
        });
        ...
        // Get all profiles from DB
        User.find({username: regex, type: 'student'}, function(err, users){
           if(err){
               console.log(err);
           } else {
              if(users.length < 1) {
		...
		var noMatch = null;
        // Get all profiles from DB
        User.find({number: req.query.number, type: 'student'}, function(err, users){
           if(err){
               console.log(err);
           } else {
              if(users.length < 1) {
		 ...
		 // Get all profiles from DB
        User.find({ type: 'student' }, function(err, profiles){
           if(err){
               console.log(err);
           } else {
              res.render("profiles",{profiles: profiles, noMatch: noMatch, user: req.user, page: "Students"});
           }
        });
    }
	});
...
app.get('/teachers', (req, res) => {
    User.find({ type: 'teacher' }, (err, profiles) => {
        if(err) throw err;
